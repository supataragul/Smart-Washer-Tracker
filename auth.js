...
module.exports = function(config) {
  ...
  function getAuthenticationUrl() {
    ...
    return client.generateAuthUrl({ scope: ['email'] });
  }

  function getAuthenticationUrlForAddDevice(tracker_id) {
    var client = new google.auth.OAuth2(
      config.oauth2.clientId,
      config.oauth2.clientSecret,
      config.oauth2.redirectUrlForAddDevice
    );
    // Use 'profile' scope to authorize fetching the user's profile
    return client.generateAuthUrl({state: tracker_id, scope: ['email']});
  }
  ...
  function getUserFromAddDevice(authorizationCode, callback) {
    var client = new google.auth.OAuth2(
      config.oauth2.clientId,
      config.oauth2.clientSecret,
      config.oauth2.redirectUrlForAddDevice
  ...
  return {
    ...
    getUser: getUser,
    getAuthenticationUrlForAddDevice: getAuthenticationUrlForAddDevice,
    getUserFromAddDevice: getUserFromAddDevice
  };
};
